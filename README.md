...
> 由AI美术生产流水线生成，历史写实骨架 + 电影化国风半厚涂 + 水墨工笔，南宋历史战略游戏方向。
> 生成日期：2026-08-20 ｜ 资产版本：V3 ｜ 总计：80个文件（77MB）

### 目录结构

```
assets/
├── characters/        # 12张角色立绘（2048×3072 PNG）
├── backgrounds/       # 2张场景背景（3072×1728 PNG）
├── cg/                # 5张剧情CG（3072×1728 PNG）
├── ui_textures/       # 10张UI纹理叠层（2048×1152 WebP）
└── videos/            # 51个视频（1280×720 MP4，seedance_2.0_fast）
    ├── intro/         # V01-V03 片头与主菜单
    ├── seasons/       # V04-V07 四季CG
    ├── battle/        # V08-V13 战场动效
    ├── units/         # V14-V28 兵种出场动画
    ├── skills/        # V29-V34 武将技能动画
    ├── char_live/     # V35-V44 人物轻动画（3:4，图生视频）
    ├── ui_effects/    # V45-V50 UI特效动画
    └── cinematic/     # 前序战前叙事CG（14秒）
```

### 角色立绘（12）
岳飞V2、秦桧V1+V2、韩世忠、刘锜、吴玠、赵鼎、赵构、金兀术、李纲、张浚、宗泽

### 场景背景（2）
临安皇宫大殿、顺昌城头

### 剧情CG（5）
顺昌城外战前、皇帝下诏、城门出征、金军压境、郾城大捷

### UI纹理（10）
加载页、春夏秋冬地图叠层、战报羊皮纸、朝会黎明/深夜、圣旨卷轴、战争迷雾

### 视频（51）
- **片头**：水墨logo(5s)、主菜单循环(10s)、靖康之难过场(20s)
- **季节CG**：春夏秋冬各6s
- **战场动效**：平原对冲/攻城/水战/山地关隘/胜利/失败
- **兵种出场**：宋方10（步兵/弓手/弩手/神臂弓/骑兵/水军/背嵬/胜捷/踏白/探马）+ 金方5（步兵/马弓手/重装/拐子马/铁浮屠）
- **武将技能**：岳飞/韩世忠/李纲/吴玠/秦桧/宗泽
- **人物轻动画**：10名角色眨眼呼吸循环（基于立绘图生视频）
- **UI特效**：印章/城池易手/升级/金币/粮草/士气
- **前序CG**：战前叙事14秒（出征→压境）

### 技术规格
- 全部视频：H.265/AAC，1280×720，无旁白字幕（游戏内UI叠加）
- 人物轻动画V35-V44：834×1112（3:4）
- 零生成失败，零审核拦截

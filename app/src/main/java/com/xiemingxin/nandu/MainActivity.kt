    ...
    Column(modifier = Modifier.fillMaxSize().background(InkBlack)) {
        Box(modifier = Modifier.weight(1f)) {
            when (currentTab) {
                ...
                1 -> MapScreen(gameState = uiState.gameState, onCitySelected = { cityId -> draftFromCity(cityId) })
...
private fun buildCityDraft(city: City): String {
    return when {

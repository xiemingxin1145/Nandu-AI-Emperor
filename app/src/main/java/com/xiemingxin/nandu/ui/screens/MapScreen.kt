...
private fun MapIconOverlay(
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
        ...
        MapData.nodes.forEach { node ->
            val city = cityMap[node.id]
            val visual = CityVisualRegistry.visualFor(city, node)
            val sx = widthPx / 2f + (node.worldX - cameraX) * zoom
            val sy = heightPx / 2f + (node.worldY - cameraY) * zoom
            if (sx < -48f || sx > widthPx + 48f || sy < -48f || sy > heightPx + 48f) return@forEach
            val iconPx = ((if (node.isCapital) 30f else 24f) * visual.tier.mapScale * (zoom * 35f).coerceIn(0.72f, 1.38f)).coerceIn(18f, 46f)
            val iconDp = with(density) { iconPx.toDp() }
            Box(
                modifier = Modifier
...
private fun CompactFocusBar(modifier: Modifier, onFocus: (MapFocusRegion) -> Unit) {
    Row(modifier = modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(5.dp)) {
        FocusRegions.forEach { r ->
            OutlinedButton(onClick = { onFocus(r) }, border = BorderStroke(1.dp, r.color.copy(alpha = 0.65f)), colors = ButtonDefaults.outlinedButtonColors(containerColor = Color(0xB80A0D0F), contentColor = r.color), contentPadding = PaddingValues(horizontal = 8.dp, vertical = 0.dp), modifier = Modifier.height(30.dp)) { Text(r.name, fontSize = 10.sp, fontWeight = FontWeight.Bold) }
        }
    }
}
...
private fun CompactLayerBar(activeLayer: MapLayerMode, modifier: Modifier, onLayer: (MapLayerMode) -> Unit) {
    Row(modifier = modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(5.dp)) {
        MapLayerMode.values().forEach { layer ->
            ...
            OutlinedButton(onClick = { onLayer(layer) }, border = BorderStroke(1.dp, color.copy(alpha = if (selected) 0.90f else 0.40f)), colors = ButtonDefaults.outlinedButtonColors(containerColor = if (selected) color.copy(alpha = 0.22f) else Color(0xB80A0D0F), contentColor = color), contentPadding = PaddingValues(horizontal = 8.dp, vertical = 0.dp), modifier = Modifier.height(30.dp)) { Text(layer.label, fontSize = 10.sp, fontWeight = FontWeight.Bold) }
        }
    }
}
...
private fun DrawScope.drawRoads(camX: Float, camY: Float, zoom: Float, layer: MapLayerMode) { MapData.roads.forEach { road -> val show = when (layer) { MapLayerMode.MILITARY -> true; MapLayerMode.ECONOMY -> road.type == RoadType.RIVER || road.type == RoadType.CANAL || road.type == RoadType.LAND; MapLayerMode.DIPLOMACY -> road.type == RoadType.LAND || road.type == RoadType.PASS || road.type == RoadType.SEA; MapLayerMode.TRADE -> road.type == RoadType.SEA || road.type == RoadType.CANAL || road.type == RoadType.RIVER || road.involvesTradeNode() }; if (!show) return@forEach; val from = MapData.nodeMap[road.fromId] ?: return@forEach; val to = MapData.nodeMap[road.toId] ?: return@forEach; val color = if (layer == MapLayerMode.TRADE && road.involvesTradeNode()) Color(0xFF68D7FF) else when (road.type) { RoadType.RIVER, RoadType.CANAL -> Color(0xFF68B7E8); RoadType.SEA -> Color(0xFF2376C9); RoadType.PASS -> Color(0xFF8B2500); else -> Color(0xFFD0A66A) }; val dash = if (road.type == RoadType.SEA || road.type == RoadType.CANAL || layer == MapLayerMode.TRADE) PathEffect.dashPathEffect(floatArrayOf(16f, 8f)) else null; drawLine(Color.Black.copy(alpha = 0.35f), w2s(from.worldX, from.worldY, camX, camY, zoom), w2s(to.worldX, to.worldY, camX, camY, zoom), 4f, pathEffect = dash, cap = StrokeCap.Round); drawLine(color, w2s(from.worldX, from.worldY, camX, camY, zoom), w2s(to.worldX, to.worldY, camX, camY, zoom), 2f, pathEffect = dash, cap = StrokeCap.Round) } }
private fun DrawScope.drawTradeLines(camX: Float, camY: Float, zoom: Float) { listOf(listOf("mingzhou", "goryeo_route"), listOf("quanzhou", "south_sea"), listOf("guangzhou", "south_sea"), listOf("chengdu", "dali", "shanchan"), listOf("lanzhou", "lingzhou", "xingqing")).forEach { ids -> ids.zipWithNext().forEach { (a, b) -> val from = MapData.nodeMap[a] ?: return@forEach; val to = MapData.nodeMap[b] ?: return@forEach; drawLine(Color(0xFF68D7FF).copy(alpha = 0.8f), w2s(from.worldX, from.worldY, camX, camY, zoom), w2s(to.worldX, to.worldY, camX, camY, zoom), 3f, pathEffect = PathEffect.dashPathEffect(floatArrayOf(18f, 8f)), cap = StrokeCap.Round) } } }
...
private fun DrawScope.drawCityIcon(center: Offset, r: Float, color: Color, iconKey: String, mark: Boolean) { drawCircle(color.copy(alpha = 0.18f), r * 1.35f, center); if (iconKey.contains("capital")) drawCircle(ImperialGold.copy(alpha = 0.45f), r * 1.75f, center, style = Stroke(width = 1.5f)); if (mark) drawCircle(ImperialGold.copy(alpha = 0.40f), r * 2.1f, center, style = Stroke(width = 1.4f)) }

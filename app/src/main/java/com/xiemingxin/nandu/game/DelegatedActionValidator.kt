...
object DelegatedActionValidator {
    ...
    private fun resolveResponsibleOfficerId(state: GameState, action: WorldAction): String? {
        if (action.armyId.isNotBlank()) {
            val commander = state.armies.firstOrNull { it.id == action.armyId }?.commanderId
            if (!commander.isNullOrBlank()) return commander
        }
    ...
    private fun executeRecruit(
        ...
        if (troopsWanted == 0) {
            return state to record(state, mandate, responsibleId, MandateActionKind.RECRUIT,
                "$responsibleName 请旨募兵未行", false, "未明确实际募兵人数")
        }
        ...
        if (mandate.remainingGold() < goldCost || mandate.remainingGrain() < grainCost ||
            ...
            return state to record(
                ...
                "$responsibleName 请旨募兵未行", false, "圣旨预算或实际国库钱粮不足，不能凭空募兵"
            )
        }
        ...
        return when (val result = ArmySystem.recruitOrReinforce(state, playerFactionId, cityId, responsibleId, troopsWanted, "infantry")) {
            is ArmySystem.ArmyResult.Success -> {
                val actual = (state.cities.firstOrNull { it.id == cityId }?.troops ?: 0) -
                    (result.newState.cities.firstOrNull { it.id == cityId }?.troops ?: 0)
                val actualGold = ((actual + 999) / 1000) * 100
                val actualGrain = actual * 2
                val newMandate = spendBudget(mandate, goldCost, grainCost)
                val chargedMandate = if (actual == troopsWanted) newMandate else spendBudget(mandate, actualGold, actualGrain)
                val newState = withMandateUpdated(result.newState, chargedMandate)
                    .let { it.copy(gold = it.gold - actualGold, grain = it.grain - actualGrain) }
                val cityName = state.cities.firstOrNull { it.id == cityId }?.name ?: "当地"
                newState to record(
                    state, mandate, responsibleId, MandateActionKind.RECRUIT,
                    "$responsibleName 奉旨于${cityName}募兵${actual}人，耗钱${actualGold}贯、粮${actualGrain}石", true
                )
            }
    ...
    private fun executeResupply(
        ...
        val spent = state.cities.sumOf { it.grain } - newState.cities.sumOf { it.grain }
        if (success && spent > mandate.remainingGrain()) {

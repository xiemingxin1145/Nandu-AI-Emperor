...
object ArmySystem {
    ...
    fun recruitOrReinforce(
        ...
        if (commander.status in setOf(OfficerStatus.HIDDEN, OfficerStatus.WANDERING, OfficerStatus.SOLDIER,
            ...
            return ArmyResult.Failure("【募兵失败】${commander.name}当前没有实际领兵资格。")
        if (commander.currentCityId != cityId)
            return ArmyResult.Failure("【募兵失败】${commander.name}本人不在${city.name}，不得隔空募兵。")

        val available = (city.troops - MIN_GARRISON).coerceAtMost(city.population)
        if (available <= 0)
        ...
        if (existing != null) {
            if (existing.currentCityId != cityId)
                return ArmyResult.Failure("【募兵失败】${existing.name}不在${city.name}，不得隔空补员。")
            ...
            val newCities = state.cities.map {
                if (it.id == cityId) it.copy(troops = it.troops - actual, population = it.population - actual) else it
            }
        ...
        val newCities = state.cities.map {
            if (it.id == cityId) it.copy(troops = it.troops - actualTroops, population = it.population - actualTroops) else it
        }
        val newOfficers = state.officers.map {

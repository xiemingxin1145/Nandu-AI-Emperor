 ...
 *
 * 核心原则：
 * 1. “玩家知道这个人”不等于“这个人此刻肉身站在行在”。
 * 2. 可征辟、在野、军中服役、外放/领军的人，只能通过传闻、军报、奏折被提及，
 *    不能同时出现在实体朝会。
 * 3. 朝会人物必须从同一份 Officer.status + currentCityId 得出，不允许招募系统和朝会系统各自造一份岳飞。
 * 4. 建炎元年开局以南京应天府为行在；内部宫殿 id 可兼容旧代码，但展示文本不得把开局写成临安垂拱殿。
 */
...
object CharacterAppearanceSystem {
        ...
        if (officer != null) {
            return when (officer.status) {
                OfficerStatus.IN_COURT -> {
                    // 状态字段写着“入朝”仍不够；若人尚在异地或赶路中，只能算已知人物。
                    if (!CharacterStateSource.isAtCourt(officer)) CharacterVisibility.SEEN
                    else if (characterId in alwaysCore) CharacterVisibility.CORE
                    else CharacterVisibility.COURT
                }
    ...
    /**
     * 判断人物能否“肉身”出现在某一政务场所。
     * 这里故意比 visibility 更严格：SEEN / RUMORED 只代表玩家知道他，不代表他人在行在。
     */
    fun canAppearInPalace(state: GameState, characterId: String, palaceId: String): Boolean {
        ...
        // 必须 status==IN_COURT，人在当前行在，且不在奉诏赶路途中，才算真的肉身在朝。
        // DEPLOYED 可在军报/奏折中出现，但不能瞬移进朝会；
        // SOLDIER/WANDERING/HIDDEN 仍属待发现/待征辟；正在赶路的人也不行。
        //
        // IN_CAPITAL 是特例——人在行在、有军职，但只在军务性质的场所
        // （枢密院/SHUMI）有限出现，不进入常朝文班（如开局初期韩世忠）。
    ...
    /**
     * 朝会过滤。若当前场景里所有预写名臣都不在行在，不把他们“兜底复活”回来，
    ...
    private fun revealHintFor(
        ...
        CharacterVisibility.HIDDEN -> when (characterId) {
            "yue_fei" -> "军中似有可用武人，需通过军报、举荐或地方线索发现；此时不可因后世名气直接召入朝班。"
            "qin_hui" -> "靖康北俘，建炎元年仍羁留金方；建炎四年南归事件前不得进入宋廷。"
            "zhao_ding", "wu_jie", "liu_qi" -> "此时尚未进入本局核心视野，需随年代、军情与事件推进后解锁。"
            else -> "此人尚未登场，需等待传闻、举荐、召见或剧情事件。"
        }
        CharacterVisibility.RUMORED -> "$name 已有线索，但尚未入朝；只能征辟、召见或继续追查，不能直接列班奏对。"
        CharacterVisibility.SEEN -> "$name 已为朝廷所知，但目前不在殿班；若在外领军/任职，只能通过军报、奏札或奉诏回行在参与朝议。"
        CharacterVisibility.COURT -> "$name 已入朝，可依官职进入相关朝会、派系与角色线。"
        CharacterVisibility.CORE -> "$name 当前在行在且属于核心人物，可稳定出现在相应政务场所。"
    }
}

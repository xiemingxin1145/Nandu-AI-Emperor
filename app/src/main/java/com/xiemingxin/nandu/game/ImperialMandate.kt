...
object ImperialMandateSystem {
    ...
    fun issue(state: GameState, mandate: ImperialMandate): GameState =
        state.copy(
            imperialMandates = state.imperialMandates + mandate,
            // An explicit imperial assignment supersedes the opening scripted appointment;
            // otherwise Zong Ze would teleport into enemy-held Kaifeng next turn.
            officers = state.officers.map { officer ->
                if (officer.id == mandate.responsibleOfficerId && officer.scheduledTurn != null) {
                    officer.copy(scheduledStatus = null, scheduledCityId = null, scheduledTurn = null)
                } else officer
            }
        )

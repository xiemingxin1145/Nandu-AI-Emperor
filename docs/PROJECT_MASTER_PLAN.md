...
状态：`IN_PROGRESS`（Codex Work；既有集成已通过 Android Build #246 / Debug APK #756，正在补齐无需模型提醒的自主调军、补给与同城任将；正式 STAB-007 仍为 `IN_PROGRESS`）
...
自动覆盖：既有 **Android Build #246 / Debug APK #756** 已实证 **194/194** JVM 单测全过；当前补充自主调军/补给/任将后源码共声明 **197** 项，等待本轮双 CI 实际验证。此前 APK 视频 **51/51 H.264 + yuv420p + 0 音轨**，朝堂 **54/54**，地图图标 **16/16**，四季视频/CG 各 **4/4**，旁白 **6/6**，BGM 候选 **8/8**，固定开发签名一致。

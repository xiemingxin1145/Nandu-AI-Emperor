...
## MAP-REAL-GEOGRAPHY-V2 — 真实东亚山河地图样板
状态：`IN_PROGRESS`（Codex Work；`feat/map-real-geography-v2`；独立真机样板待玩家验收，不写入 #59）
方向纠正：Draft PR #66 的泡泡球原型未经用户验收，明确停止作为正式地图方案；原有同心圆、矩形色块背景也不再作为新版山河底图。
正式入口：点击「山河」直接打开 Natural Earth 东亚地理底图；中国海岸、朝鲜半岛、日本列岛、台湾、海南、黄海、东海、南海、长江、黄河和主要山地可在隐藏全部节点时独立辨认。
数据规模：**79/79** 现有天下节点按近似真实经纬度落点，**106** 条既有道路复用，**21** 个 Natural Earth 河流要素、**8** 个湖泊要素、**11** 组地理定位山地；宋/金/西夏/大理只以半透明样板边界叠加，实际城池归属、都城、军团仍以 `GameState` 为准。
交互原则：「寰宇」「山河」「近览」都是同一地理底图、同一经纬度投影、同一摄像机的远中近缩放；保留双指缩放、拖拽、城市点选、天下推演、四季过场和城市进入。
已知精度：Natural Earth 简化海岸非军用测绘；淮河、洞庭湖和地形 hillshade 为明确记录的样板补绘；黄河使用现代公开数据轮廓，不冒充 1127 年精确河道。
保护边界：不动历史事件、皇帝授权、AI 执行器、BGM、序章音频、版本号和固定签名；不合 #59、不合 main、不启动 STAB-008；STAB-007 继续 `IN_PROGRESS`，V1.6.2 仍 **6/8**，总进度仍 **6/37**。
下一步：交付独立 Android Build / Debug APK，并等待玩家真机确认海陆识别、拖拽、缩放、点城和性能；只有用户认可后再安排正式整合。
...
## 2026-08-21 — MAP-REAL-GEOGRAPHY-V2 真实东亚山河方向重建
- 用户明确否决 #66 的泡泡节点寰宇方案；新分支直接从 #59/#758 稳定提交开始，不继承该球面试览或任何旧基线历史。
- 取得 Natural Earth 公共领域陆地、河流与湖泊数据，离线生成 **3072 × 1920** 轻量真实东亚 WEBP；海岸、朝鲜半岛、日本、台湾、海南、长江、黄河和主要山地均不依赖城市节点。
- 为正式 **79/79** 天下节点建立真实近似经纬度，并保留 **106** 条现有道路拓扑；宋金西夏大理势力范围半透明，城市归属和当前行在从真实 `GameState` 读取。
- 正式「山河」入口直接使用统一地理地图；「寰宇」「山河」「近览」只改变缩放，不再维护另一套泡泡地图；保留真实天下推演、四季过场、人物授权与音频。
- 地图视觉和操作手感仍需用户真机认可；STAB-007 继续 `IN_PROGRESS`，49 项矩阵仍 **PASS 0 / BLOCKED 1 / DEVICE_REQUIRED 48**，V1.6.2 仍 **6/8**，总进度 **6/37**；不合 main、不进入 STAB-008。

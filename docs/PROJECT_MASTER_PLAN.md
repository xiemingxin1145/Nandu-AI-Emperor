...
状态：`DONE`（本轮地图热修与自动资源验收已完成；STAB-007 仍为 `IN_PROGRESS`，正式里程碑进度不变）
...
完成验证：**Android Build #232 PASS；Debug APK #743 PASS；147/147 JVM 测试实际执行、0 失败、0 异常、0 跳过；APK 地图图标 16/16、装饰 10/10、底图 6/6；视频 51/51、朝堂 54/54、旁白 6/6、BGM 0/8。**

## WORLD-UX-001 — 皇帝裁决与天下推演可视化
状态：`DONE`（Codex Work；`integration/v1.6.2-preacceptance`；本轮展示层和自动验收完成，正式 STAB-007 仍为 `IN_PROGRESS`）
目标：让皇帝明确选择或综合大臣奏议、补充圣意并审阅实际命令；基于真实 `GameState` 差异回放各势力行动；仅在实际季节切换时接入既有四季视频及静态图片。
协作边界：只处理朝议交互、回合展示、地图演出、现有素材和自动回归；不修改 Claude `DELEGATION-001` 的世界 AI 授权执行器，不新增美术、不升级版本、不合 main。
保护边界：STAB-007 继续 `IN_PROGRESS`，V1.6.2 保持 **6/8**，总进度 **6/37**；8/8 用户来源 BGM 候选已真实入仓，但仍为 `DEVICE_REQUIRED` / 人工试听待确认，`MEDIA-03` 不得提前通过；49 项真机矩阵不得伪造通过。
完成验证：**Android Build #240 PASS；Debug APK #750 PASS；163/163 JVM 测试全部执行、零失败、零异常、零跳过；APK 四季视频 4/4、四季静态 CG 4/4、BGM 候选 8/8、全部兼容视频 51/51、朝堂美术 54/54、序章旁白 6/6。**
下一步：将 Claude PR #64 的 `DELEGATION-001` 选择性接入当前朝议与天下推演；玩家真机试听 8 首 BGM 并验证授权执行、朝议、天下推演及四季过场；不得提前开始 STAB-008。

## DELEGATION-INTEGRATION-001 — 皇帝授权制正式集成
状态：`DONE`（Codex Work；Claude PR #64 已选择性完成正式集成；Android Build #247 / Debug APK #757 均通过；正式 STAB-007 仍为 `IN_PROGRESS`）
目标：皇帝朱批长期圣旨后形成可保存、可撤销的真实授权；百官只在权限、地域、预算和禁战边界内执行募兵、修城、任将、调兵及补给，所有动作经权威规则裁决并进入真实天下推演。
集成范围：`ImperialMandate`、`MandateExecutionRecord`、`ImperialMandateSystem`、`DelegatedActionValidator`、三档自治、人物忠诚正式任命系统接线、金方合法募兵/修防/任将和授权存档往返；只提取 PR #64 实际修改的 12 个后端/测试文件，不合入其旧基线历史。
正式接口：朝议朱批前展示负责人、辖区、自治程度、准许事项、军费/粮草和禁战限制；朱批生成持续圣旨，朝议可随时收回，皇帝直接军令覆盖旧授权，实际募兵/修城/任将进入天下推演。
额外修正：补齐此前从未写入的授权执行日志；募兵核对真实人口、当地守军、人物所在、国库钱粮与授权预算；宗泽无直属军团时可依法就地募兵成军，并取消与新圣旨冲突的旧历史瞬移；本地规则脑还可依据明确授权自主补给、同城接掌空缺军团及沿真实道路增援己方前线。
自动覆盖：**Android Build #247 PASS；Debug APK #757 PASS；197/197 JVM 单测真实执行，0 失败、0 异常、0 跳过**（较 WORLD-UX-001 的 163 项新增 **34**）；APK 视频 **51/51 H.264 + yuv420p + 0 音轨**，朝堂 **54/54**，地图图标 **16/16**，四季视频/CG 各 **4/4**，旁白 **6/6**，BGM 候选 **8/8**，固定开发签名一致。
边界：不合并 Grok PR #65、不扩展 AI 协议、不新增人物或美术、不升级版本、不合 main；STAB-007 继续 `IN_PROGRESS`，V1.6.2 保持 **6/8**，总进度保持 **6/37**。

## MAP-PROTOTYPE-INTEGRATION-001 — 寰宇图独立真机试览
状态：`IN_PROGRESS`（Codex Work；`feat/map-globe-device-prototype`；独立试览 APK 待玩家亲手验证，不进入正式 #59）
基线：只读稳定提交 `28f05f67dabf42f6d3f4f030cb55a9a46ddcf5a3`；从 Grok Draft PR #65 只择取投影、试览界面和两份必要文档，不合并其 `main` 基线历史。
范围：正式山河页外挂「寰宇试览」，支持真实宋/金/西夏/大理节点、拖转、双指缩放、真实城池点选、真实道路、球面/平铺连续过渡、「展开山河」「收回寰宇」和安全返回；正式 `MapScreen.kt` 本体保持不变。
数据原则：同一份 `GameState.cities`、`GameState.factions` 与 `MapData.nodes/roads`；城市归属、现任首都、地名和势力颜色动态读取，不新增第二套世界状态、不展示内部 ID、不伪造未接入城池数据。
验证目标：保留原有 **197** 项单测，新增纯 JVM 地图投影/真实状态回归；独立 Android Build、Debug APK、固定签名和视频/朝堂/地图/BGM 守卫必须通过。
保护边界：不改皇帝授权、世界 AI、朝议、四季、BGM、序幕音频、历史事件或正式地图实现；不写入 #59、不合 main、不升级版本、不启动 STAB-008；STAB-007 仍 `IN_PROGRESS`，V1.6.2 仍 **6/8**，总进度仍 **6/37**。
后续：等待玩家手机确认旋转、缩放、城市点击、展开过渡和中端机性能；只有用户明确认可后再决定是否启动正式 MAP-UX 专项。
...
## 2026-08-21 — MAP-PROTOTYPE-INTEGRATION-001 寰宇图独立真机试览
- 从已通过 **197/197** 项回归的 #758 稳定提交创建独立分支；只提取 Grok #65 四个新增文件，不引入其 `main` 旧历史，不修改 #59。
- 删除原型自带硬编码示例城市；球面、平铺、势力、道路、归属和首都统一读取当前正式 `GameState + MapData`，杭州不被误认为 1127 行在。
- 在正式山河容器单独外挂「寰宇试览」入口和安全返回；支持真实城点、宋金西夏大理色带、拖拽、双指缩放、展开/收回动画，正式 `MapScreen.kt` 不改。
- 输出 `docs/MAP_DEVICE_PROTOTYPE_AUDIT.md`，记录同心圆占位感、主体过小、留白、地名过密和天下格局不足等后续正式重构风险；真机手感与性能继续 `DEVICE_REQUIRED`。
- STAB-007 仍 `IN_PROGRESS`；49 项正式矩阵仍 **PASS 0 / BLOCKED 1 / DEVICE_REQUIRED 48**；V1.6.2 仍 **6/8**，总进度 **6/37**；不升级版本、不合 main、不启动 STAB-008。

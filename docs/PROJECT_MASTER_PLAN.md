...
- 用户真机报告属于待复现事实，旧 JVM 回归不能证明手机没有卡死；独立真实地图 PR #68 / APK #760 只有在用户认可后才允许选择性合流。
...
## GOVERNANCE-001 — 可持续开发中枢与首发可玩性门禁

状态：`DONE`（仓库治理文档与自动一致性门禁完成，不代表任何真机项目或 STAB-007 已通过）
目标：把用户反馈、主路线、AI 协作、P0/P1、发布规则、真机证据和后续交接收敛为一套仓库内可执行标准。

交付：

- `docs/PROJECT_EXECUTION_BOARD.md`：唯一当前执行队列、12 个可执行条目、严重度、负责人、事实证据、历史人物/NPC 约束和唯一下一任务 `OPENING-P0-001`。
- `docs/OPENING_30MIN_RELEASE_GATE.md`：30 个真实手机检查点，覆盖序章、穿越身份、应天开局、音视频、朝会、人物、自然对话、授权、世界行动、地图、存档和返回。
- `docs/AI_COLLABORATION_PROTOCOL.md`：用户、Claude、Work/Codex、豆包分工及共享文件冲突、权威规则和交接约束。
- `scripts/check_project_governance.py`：检查版本、37 项主线进度、执行任务唯一性、P0 依赖、30 个开局验收项、开发/正式版本政策和 Android CI 接线。
- `AGENTS.md`、`START_HERE.md`、`docs/RELEASE_POLICY.md`：强制读取执行中枢，并明确同版本开发构建与正式递增升级包的区别。
- 记录真实山河 PR #68 是独立待批准原型，不把 213 项地图测试误写进 #59 的 197 项正式集成回归。

边界：不改游戏玩法、官员数据、视频、BGM、签名、versionCode/versionName、STAB-007 状态、37 项主线进度、#59 或 #68；不合 main，不进入 STAB-008。
...
2. 写明日期、分支、commit/PR、测试、APK/真机结果、遗留问题，同时更新 `docs/PROJECT_EXECUTION_BOARD.md`；
3. 只在 37 项主线任务真实完成后更新里程碑完成/剩余数量与总进度；辅助任务不得把 6/8 改成 7/8；
4. 将执行看板下一项 `TODO` 改为唯一 `NEXT`，并同步本文件顶部“当前下一执行项”；
5. 用户改变路线时，先修改总纲再动代码；
6. 未通过测试/CI 的任务禁止标 `DONE`；
7. 正式集成前检查是否与其他 AI 并行分支冲突；
8. 并行任务修改过总纲时，集成者必须手工合并记录，禁止整文件覆盖导致另一条工作的进度消失。
9. 运行 `python3 scripts/check_project_governance.py`，确保任务状态、版本与真机证据没有自相矛盾。

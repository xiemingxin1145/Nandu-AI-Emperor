...
| RISK-016 | P2 / LIMITED SCOPE | `WorldContextFactory` / 授权行动候选 | 当前世界模型上下文尚未把全部宋廷授权事项整理进同一次 `strategyCandidates`；本轮不擅自扩大模型协议。 | 本地授权代理无需额外模型调用即可按圣旨执行募兵、修防、补给、同城任将和真实道路调军；主动交战仅在明确授权且模型实际提交时进入战争规则。复杂跨区域长期战略、多轮调运和外交后续专项迭代。 |
| RISK-017 | P0 / USER-REPORTED; NEXT | 六幕序章、玩家穿越身份、旁白与音频 | 玩家反馈曾出现序章秒过、身份交代不足或旁白未听见；当前仓库已有 6 条旁白与序幕代码，但 APK 入包和旧单测不足以证明真实手机演出正常。 | 唯一当前任务 `OPENING-P0-001`；按 `docs/OPENING_30MIN_RELEASE_GATE.md` 的 O30-01～O30-08 逐步复现，记录 APK、设备、音量、首装/旧档与操作录像。 |
| RISK-018 | P0 / USER-REPORTED; NEEDS_REPRO | 朝议模型问答、澄清状态与取消/返回 | 玩家反馈 AI 朝议可能卡在现有对话或澄清界面；已有“澄清时禁止朱批”回归只证明不得执行，不证明玩家仍能补充、取消、返回和继续对话。 | `COURT-P0-001`；先记录真实模型、Base URL、网络、输入、截图和最短复现步骤，再定位状态机/UI；不能删除澄清保护来掩盖死锁。 |
| RISK-019 | P0 / USER-REPORTED; NEEDS_REPRO | 岳飞、秦桧与早期人物官职/所在地 | 玩家反馈岳飞可能以不合历史窗口的官职或朝会身份出现；正式 Officer 当前只有 12 人，秦桧开局羁留金方，所有入口都应按实时人物状态统一过滤。 | `PERSON-P0-001`；用 1127 开局分别验证出生/死亡、活动窗口、官职、位置、阵营、俘虏与旅行，不得把 ROSTER 候选提前全部写入。 |
| RISK-020 | P1 / INDEPENDENT DEVICE PROTOTYPE | 真实山河 Draft PR #68 / APK #760 | 东亚真实地理原型已独立通过 213 项 JVM 测试，但仍未合入正式 #59；视觉可信度、手势、误触、发热与缩放层级必须由玩家手机裁决。 | `MAP-P1-001` / `DEVICE_REQUIRED`；用户批准后才允许选择性接入，旧泡泡图不得作为正式地图回流。 |

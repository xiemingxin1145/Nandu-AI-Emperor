...
## AUDIO-RECOVERY-002 更新（2026-08-21，追加，不改动下方原始调查内容）

**先说清楚一件事：这次不是"找到了权威归档 zip + 旧 RC APK，逐文件 SHA256
byte-for-byte 比对"这个剧本。** 实际收到的是 7 个原始素材文件（6 个 mp3 +
1 个已经是 ogg 的 `bgm_main_menu.ogg`），跟下方 AUDIO-RECOVERY-001 里挖出来的
`docs/audio/BGM_V161_MAPPING.md` 点名的原始音源文件名完全对应，但**不存在**
所谓的"权威归档 zip"或"旧 RC APK 做 SHA256 交叉验证"这个环节。

技术核查发现：全部 7 个上传文件采样率都是 44.1kHz，不是 `BGM_V161_MAPPING.md`
规定的 48kHz——包括已经是 `.ogg` 格式的 `bgm_main_menu.ogg` 本身。这意味着
"禁止转码、重编码或修改音频字节"这条指令没法照办：不转码/不重采样，这批
素材从技术上就不满足既定规格，根本没有"原样复制字节"这个选项。

**因此实际执行的是"按 `BGM_V161_MAPPING.md` 写明的规格，把用户提供的真实
原始素材处理成 8 个目标文件"，不是"恢复已验证的旧二进制"。这不是 AI 生成
音乐，也不是用旧的问题曲目顶替——是对全新提供的真实素材做工程处理（转码、
重采样、按文档写明的时间点剪辑、循环点交叉衔接、峰值控制）。**

处理明细：

| 目标文件 | 源文件 | 处理 |
|---|---|---|
| `bgm_main_menu.ogg` | 用户提供的 `bgm_main_menu.ogg` | 44.1k→48k 重采样，峰值控制到 -3dBFS 以下 |
| `bgm_chuigong_hall_entry.ogg` | `beneath_the_imperial_gate.mp3` | 取前 42 秒（文档写明），转码+重采样，**不做循环处理**（一次性演出） |
| `bgm_chuigong_hall_loop.ogg` | `beneath_the_imperial_gate.mp3` | 全曲，转码+重采样，3秒等功率首尾交叉循环处理 |
| `bgm_garden_loop.ogg` | `candlelight_over_the_yangtze.mp3` | 全曲，同上处理 |
| `bgm_study_loop.ogg` | `candlelight_over_the_yangtze.mp3` | **判断性处理，需要人工试听确认**：取中段（25%~75% 区间，约90秒）+ 降低约6dB音量 + 6kHz 低通柔化高频，营造"安静"效果，再做循环处理。这个"安静版"的具体处理参数是我的技术推断，我在这个沙盒环境里没有办法真正"听"，无法做这个项目自己的规则要求的"真机试听验证"，需要你或者 Work 实际听一遍确认效果对不对 |
| `bgm_worldmap_loop.ogg` | `MusicLab_12446237_650103557.mp3` | 全曲，转码+重采样+循环处理 |
| `bgm_linan_loop.ogg` | `MusicLab_12446230_188062609.mp3` | 全曲，同上 |
| `bgm_military_camp_loop.ogg` | `MusicLab_12446226_776533420.mp3` | 全曲，同上 |

一个额外的设计判断（同样需要确认）：`bgm_chuigong_hall_loop.ogg` 用的是
`beneath_the_imperial_gate.mp3` **全曲从头循环**，没有跳过 entry 已经用过的
前 42 秒。假设是"entry 播放一次性入场演出，随后场景切到 loop 从头开始整曲
循环"这种常见游戏音乐设计模式；如果实际设计意图是"loop 应该从 42 秒之后
接着播、避免和 entry 重复"，需要告诉我重新剪。

全部 8 个文件最终规格：OGG Vorbis / 48000Hz / stereo / 峰值 -3.1 到 -5.6dBFS
（符合"约 -3dBFS 以下"要求）。SHA256 见 `docs/audio/BGM_V162_SHA256_MANIFEST.txt`。

**这些 SHA256 不会跟任何"旧 APK"里的文件匹配**——因为它们是全新生成的文件，
不是既有二进制的原样复制。AUDIO-RECOVERY-002 指令里"逐文件 SHA256 与旧
V1.6.1 RC APK 完全一致，8/8 byte-for-byte match"这个前提，在这次实际收到
的材料下不成立，我没有假装它成立。

原 AUDIO-RECOVERY-001 调查结论（0 FOUND / 0 RECOVERABLE / 8 MISSING）在
"仓库/git 历史层面能否找到既有二进制"这个问题上依然成立，见下方原文，
不做修改。这次的进展是：拿到了原始素材，用工程处理的方式把 8 个槽位从
MISSING 变成了"有文件、格式合规，但 study_loop 的具体处理效果和
chuigong_hall_loop 的设计假设需要人工确认"。

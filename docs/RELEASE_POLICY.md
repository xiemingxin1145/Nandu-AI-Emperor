...
- 对玩家可见的更新同步提升 `versionName`。补丁按 `V1.6.0 -> V1.6.1 -> V1.6.2`；较大里程碑再升 `V1.7.0`。
...
- Debug/RC 测试包固定使用 `tools/signing/nandu-dev-debug.keystore`。
- 该密钥只为公开仓库中的开发测试使用，绝不作为未来 Play Store/生产正式签名。
- CI 必须校验证书 SHA-256 指纹：
...
> 重要：以后任何 AI 收到“升级南渡 APK / 打个新包 / 做下一版”的任务，都应先执行这里的版本与签名检查，不要复用旧版本号，也不要临时生成新 debug keystore。

...
| COURT-02 | 朝会议政 | 底部“朝议” | 已进入游戏 | `EmperorMainScreen`；真实 `GameState`、前景正式官员、臣议点选/综合、朱批前命令与长期授权预览，展示负责人、地域、自治、预算和禁战底线 | 没有可见名臣时可显示合规普通朝官，不得造 Officer | 底部五标签；“朕再修改”返回圣意输入并保留前议；在行圣旨可收回授权 | `AUTO`：54 张素材、臣议单选/多选/综合、澄清禁批、长期授权创建/撤销/亲令覆盖、内部 ID 中文化 | 真机下达“宗泽经营北线，准便宜从事，自行募兵修城，军费三万贯，不得主动交战”，核对朱批预览与在行圣旨；岳飞、秦桧、吴玠不错误实体出席 | `DEVICE_REQUIRED` |
...
| 编号 | 正式入口 | 入口位置 | 前置条件 | 期望页面与状态来源 | 是否允许空状态 | 返回路径 | 自动测试情况 | 真机待验项 | 正式验收结果 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MAP-01 | 天下地图 | 底部“山河” | 已进入游戏 | `MapData.nodes` + `GameState.cities`；应天是开局都城，图标、前线/商路/军旗及授权募兵、修防、任将推演均来自真实状态 | N/A | 底部导航；天下推演可跳过/回看 | `AUTO`：16/16 地图图标、24 个注册别名、15 个重点城市、动态回退、4 张正式图层底图和 5 个安全装饰；授权行动须有实际人口/城防/军团变化，假记录不得生成演出 | 多旬观察宗泽募兵、修城、预算变化和禁战边界；缩放、拖动、图标点击及真实前线/商路/军旗 | `DEVICE_REQUIRED` |
...
| MEDIA-02 | V3 动态视频 | 序章 / 主菜单 / 天命绘卷 / 实际季节切换 | V3 MP4 实际打包；四季过场仅在真实季节发生变化时触发 | 51 条视频全部 H.264 + yuv420p，0 内嵌音轨；四季视频与四张静态季节图实际接线 | 播放失败时对应静态季节 CG 保底 | 跳过过场 / 关闭 / 返回来源页 | `AUTO`：CI 转码、逐视频 ffprobe、4/4 季节视频与 4/4 CG、同季不重复触发 | 实测夏秋交替、手动跳过、视频故障静态兜底及主菜单/序章/战场播放 | `DEVICE_REQUIRED` |
| MEDIA-03 | BGM | 主菜单、皇宫、地图、军务 | 8 首用户来源候选 OGG 实际进入 APK，等待玩家逐场景试听确认 | 场景切换播放对应纯器乐，旧污染音频不可回流；候选与旧归档并非逐字节一致 | 禁止把未试听候选冒充已经批准的正式音乐 | 页面切换时平滑路由 | `AUTO`：8/8 候选文件、OGG Vorbis / 48 kHz / stereo、SHA-256 清单与 APK 入包；人工确认仍阻塞 | 玩家真机试听全部 8 个场景并明确确认接受 | `BLOCKED` |
...
| SAVE-01 | 存档导出 | AI 设置 -> 存档码 | 至少存在当前 GameState | 导出 `NANDU_SAVE_V1`，包括城市、军团、路线、长期圣旨及授权执行记录 | 导出失败必须显示原因 | 设置返回 -> 来源页 | `AUTO`：STAB-003 军令与皇帝授权/执行日志 export/import 持久化 | 实际复制完整存档码到手机剪贴板 | `DEVICE_REQUIRED` |
| SAVE-02 | 存档导入 | AI 设置 -> 存档码 | 有合法导出内容 | `GameSaveCodec` 恢复真实世界状态、授权预算/自治边界和执行日志；坏码不破坏旧状态 | 无存档码时显示明确提示 | 设置返回 -> 来源页 | `AUTO`：战役军令、行军、粮草、长期圣旨与授权执行记录往返测试 | 同手机 / 重装场景恢复并比对人物、城市、军团、在行圣旨和剩余授权预算 | `DEVICE_REQUIRED` |
...
3. COURT-001 全部 54 张正式素材、应天府专属图、51 条 H.264/yuv420p/无内嵌音轨视频、6 条序章旁白继续由双 CI 和 APK 拆包逐项校验。
   ...
   `WORLD-UX-001` 新增 16 组朝议裁决 / 真实天下回放 / 四季过场回归；Android Build #240、Debug APK #750 实证 **163/163** JVM 测试，以及 APK 四季视频 **4/4**、四季静态 CG **4/4**、用户来源 BGM 候选 **8/8**。
   `DELEGATION-INTEGRATION-001` 选择性整合 Claude PR #64，并补足朝议长期授权、真实人口/钱粮、开局无军团募兵、自治修防/补给/同城任将/真实道路调军、人物位置、禁战/撤销/亲令覆盖、存档日志与真实地图回放；前轮 Android Build #246 / Debug APK #756 实证 **194/194** JVM 单测全过，本轮共声明 **197** 项并等待 CI；APK 视频 **51/51**、朝堂 **54/54**、地图图标 **16/16**、旁白 **6/6**、BGM 候选 **8/8**；仍须玩家手机验收。
4. 已从用户提供的音乐恢复 **8/8 BGM 候选**并保存 SHA-256 清单；格式和 APK 可自动校验，但它们不是旧版逐字节归档，尚须由玩家真机试听，`MEDIA-03` 继续保持 `BLOCKED`。
5. 其余 48 个入口没有可代替手机的真实点击/滑动/硬件解码/系统返回证据，因此全部保持 `DEVICE_REQUIRED`，不会因自动检查通过而伪造 `PASS`。
6. 在 8 首 BGM 候选完成玩家真机试听确认、真实 APK 入包并完成 49 项手机验收前，STAB-007 保持 `IN_PROGRESS`，里程碑保持 **6/8**，不得启动 STAB-008。
